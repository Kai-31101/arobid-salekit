...
const rawDemoScriptSteps: DemoJourneyStep[] = [
    ...
    { actor: 'Partner', action: 'Chọn company profile', screen: 'Invite Exhibitor Modal', path: `/partner/operation/expos/${eid}`, cta: 'Verified supplier', prep: ['Invite Exhibitor'],
      script: 'Modal mở mặc định ở tab Arobid. Partner đi qua toàn bộ bước nội bộ Arobid trước: chọn các company profile đã có sẵn trong hệ thống, check/uncheck nhiều công ty trong danh sách.',
      scriptEn: 'The modal opens on the Arobid tab by default. The Partner goes through the Arobid-internal steps first: picking company profiles already on Arobid, checking or unchecking several companies in the list.' },
    { actor: 'Partner', action: 'Switch Arobid / External', screen: 'Invite Exhibitor Modal', path: `/partner/operation/expos/${eid}`, cta: 'External', prep: ['Invite Exhibitor'],
      script: 'Xong với danh sách Arobid, Partner mới chuyển sang tab External để mời các doanh nghiệp bên ngoài bằng email.',
      scriptEn: 'Only after finishing with the Arobid list does the Partner switch to the External tab to invite outside businesses by email.' },
